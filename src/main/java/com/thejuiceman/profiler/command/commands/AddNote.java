...
public class AddNote implements Command {
    ...
    public boolean execute(CommandHandler cmdHandler, CommandSender sender, String... args) throws Exception {
        ...
        Profiler profiler = cmdHandler.getProfiler();
        MySQL mysql = cmdHandler.getProfiler().getMysql();

        Bukkit.getScheduler().runTaskAsynchronously(profiler, new Runnable() {
            @Override
            public void run() {
                ...
                try{
                   ...
                   // Check if player is adding note to self
                    ...
                    sender.sendMessage(ChatColor.GOLD + "Successfully added note to player: " + ChatColor.WHITE + playerNameFormat);
                    return;
